...
class Admin::CategoriesController < AdminController
  ...
  def index
    @admin_categories = if current_user.has_role? :superadmin
                          Admin::Category.all
                        else
                          Admin::Category.where(user_id: current_user.id)
                        end

    if request.xhr?
      @admin_categories = if params[:user_id].present?
                            Admin::Category.where(user_id: params[:user_id])
  ...
  def edit
    return if @admin_category.image.present?
  ...
  def admin_category_params
    ...
    # end
    params[:admin_category][:ancestry] = nil if params[:admin_category][:ancestry] == ''
    params.require(:admin_category).permit(:name, :ancestry, image_attributes: %i[name avatar])
  end
end
